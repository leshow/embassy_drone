    ...
    TRANSPORT: Transport,
{
    ...
    /// Configure the INT1 pin as active-high, push-pull, latched until cleared by any
    /// register read, and enable the raw-data-ready interrupt source. Lets a caller wait on
    /// the physical INT pin instead of polling `new_data_ready`.
    pub async fn enable_data_ready_interrupt(&mut self) -> Result<(), TRANSPORT::Error> {
        // INT_PIN_CFG: INT1_LATCH_EN (bit 5) + INT_ANYRD_2CLEAR (bit 4), INT1_ACTL/INT1_OPEN clear
        self.write_to(Bank0::IntPinCfg, 0b0011_0000).await?;
        // INT_ENABLE_1: RAW_DATA_0_RDY_EN (bit 0)
        self.write_to(Bank0::IntEnable1, 0b0000_0001).await?;
        Ok(())
    }
...
// SPI-only: the SDA/SCL pins double as SDI/SCLK, so the datasheet requires disabling the I2C
// slave interface to stop it from mistaking SPI traffic for I2C. Scoped to SpiDevice rather than
// the generic Transport impl above so calling this on the I2C path is a compile error, not just a
// documented footgun - datasheet section 6.1: "To prevent switching into I2C mode when using SPI,
// the I2C interface should be disabled by setting the I2C_IF_DIS configuration bit."
impl<SPI, MAG> Icm20948<SpiDevice<SPI>, MAG>
where
    SPI: spi::SpiDevice,
{
    /// Disables the I2C slave interface (USER_CTRL.I2C_IF_DIS, bit 4), locking the shared pins
    /// into SPI-only mode. Call once, right after initializing. Uses the same read-modify-write
    /// as the rest of the driver's flag writes, so it doesn't disturb I2C_MST_EN (bit 5, used for
    /// the magnetometer's internal I2C bridge) or any other USER_CTRL bit.
    pub async fn disable_i2c_interface(&mut self) -> Result<(), SPI::Error> {
        self.write_to_flag(Bank0::UserCtrl, 1 << 4, 1 << 4).await
    }
}
